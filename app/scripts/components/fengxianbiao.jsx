...
class Fengxianbiao extends React.Component {
  
  constructor(props){
    super(props);
    this.state = {
      num: parseInt(props.fnum),
      secAngle0:18.2*props.fnum-90,
      secAngle:18.2*props.fnum-90,
  ...
  subtract(e){
    if(this.state.num>1){
      ...
      var angel = 18.2*i-90;
      this.setState({num: i, secAngle: angel});
    }
  }
  add(e){
    if(this.state.num<9){
  ...
  zhankai(i){
    ...
    if(i==this.state.zhankai){
      ...
      if(i>2){
        this.setState({xiahua:true});
      }
    }
  }
  componentDidUpdate(){
    if(this.state.xiahua){
      var i=0;
  ...
  render() {
    var result = this.props.result=="1"?1:0;
    var fnum = this.state.num-1,
        list = this.state.touzixiang.map((item,index) => this.state.peizhi[fnum][index]!=0?
                <li key={index}>
                  <div onClick={this.zhankai.bind(this,index+1)}><span className={"yuanquan y"+index} style={{background: this.state.colors[index]}}></span>{this.state.peizhi[fnum][index]}%<span className="liubai">{item}</span>
                  {this.state.zhankai==index+1?<img src="./images/g1.png" className="g1"/>:<img src="./images/g2.png" className="g1"/>}
                  </div>
    ...
    return (
      <div>
        <div className="panzi">
          <img src="./images/beitu.png" className="beitu"/>
          {result?<img src="./images/zhen2.png" className="zhen" style={{transform: 'rotate(' + this.state.secAngle0 + 'deg)'}}/>:''}
          <img src="./images/zhen.png" className="zhen" style={{transform: 'rotate(' + this.state.secAngle + 'deg)'}}/>
        </div>
        <div className="shuzi">
          <i onTouchStart={ this.subtract.bind(this) }><img src="./images/jianhao.png" className="g1"/></i>
          <i>{this.state.num}</i>
          <i onTouchStart={ this.add.bind(this) }><img src="./images/jiahao.png" className="g1"/></i>
        </div>
        {result?<p className="yuanlai">原{this.props.fnum}.0</p>:''}
        <div className="zhuzi">
        {zhuzi}
        </div>
        <ul className="list">
